...
# ==============================================================================
# CELL 40 — EXPLORATORY VALIDATION GATE CASE STUDY
# ==============================================================================
#
# Purpose
# -------
# Provide three deterministic validation examples illustrating low, median,
# and high values of SAVRec's learned evidence-guided visual gate.
#
# Selection rule
# --------------
# Choose the validation target pairs nearest to the:
#   - 10th percentile of the three-seed mean gate
#   - 50th percentile of the three-seed mean gate
#   - 90th percentile of the three-seed mean gate
#
# IMPORTANT
# ---------
...
# - NO manual case selection / cherry-picking.
# - Uses already-saved Cell-39 gate values if the notebook state is unavailable.
#
# ==============================================================================
...
print("=" * 100)
print("CELL 40 — EXPLORATORY VALIDATION GATE CASE STUDY")
print("=" * 100)
...
# ==============================================================================
#
# Prefer the variables defined by Cell 1. If this cell is run after a runtime
# restart, reconstruct the same paths from SAVREC_DATA_ROOT / default Drive path.
# ==============================================================================

DATA_ROOT_40 = globals().get(
    "DRIVE_PATH",
    os.environ.get(
        "SAVREC_DATA_ROOT",
        "/content/drive/MyDrive/MS_new/Thesis_HotelRec_Data"
...
EXPERIMENT_ROOT_40 = globals().get(
    "EXPERIMENT_ROOT",
    os.path.join(
        DATA_ROOT_40,
...
assert SEEDS_40 == [42, 1, 7], SEEDS_40


GATE_CACHE_PATH = os.path.join(
    RESULT_DIR_40,
    "exploratory_validation_target_gate_per_user_v1.csv"
)

ITEM_EVIDENCE_PATH = os.path.join(
    ARTIFACT_DIR_40,
    "clean_final_evidence_item_features.csv"
)

TRAIN_FILE_40 = globals().get(
    "TRAIN_FILE",
    os.path.join(
        DATA_ROOT_40,
        "train_v6.parquet"
...
CASE_FULL_PATH = os.path.join(
    RESULT_DIR_40,
    "exploratory_validation_gate_case_study_full_v1.csv"
)

CASE_PAPER_PATH = os.path.join(
    RESULT_DIR_40,
    "exploratory_validation_gate_case_study_paper_v1.csv"
)

CASE_MANIFEST_PATH = os.path.join(
    ARTIFACT_DIR_40,
    "cell40_gate_case_study_manifest_v1.json"
)
...
# ==============================================================================
# 2. LOAD / REUSE CELL-39 GATE VALUES
# ==============================================================================
#
# Sequential notebook run:
#     reuse gate_seed_df already produced by Cell 39.
#
# Restarted runtime:
#     load the CSV saved by Cell 39.
# ==============================================================================

if "gate_seed_df" in globals():

    gate_seed_df_40 = (
        globals()["gate_seed_df"]
        .copy()
    )
    ...
    if not os.path.exists(
        ...
        raise FileNotFoundError(
            "Cell-39 gate cache was not found:\n"
            f"{GATE_CACHE_PATH}\n\n"
            "Run Cell 39 once. "
            "No model retraining is required."
        )

    gate_seed_df_40 = pd.read_csv(
        GATE_CACHE_PATH
    )
...
# Every validation target pair must have one gate value for every SAVRec seed.
...
print(
    f"Gate rows          : "
    f"{len(gate_seed_df_40):,}"
)

print(
    f"Validation targets : "
    f"{len(pair_seed_counts_40):,}"
)


# ==============================================================================
# 3. THREE-SEED TARGET-PAIR GATE STATISTICS
# ==============================================================================
...
# ==============================================================================
# 4. USER TRAINING-HISTORY EVIDENCE
# ==============================================================================
#
# Prefer the already-clean V7 train_df if available.
# Otherwise reload train_v6.parquet and apply the same invalid-user cleaning rule.
# ==============================================================================
...
if "train_df" in globals():
    ...
    raw_train_df_40 = pd.read_parquet(
        TRAIN_FILE_40
    )

    invalid_user_ids_40 = set(
        globals().get(
            "INVALID_USER_IDS",
            {
                "",
...
print(
    f"Clean train interactions : "
    f"{len(train_df_40):,}"
)

print(
    f"Training users           : "
    f"{len(user_history_40):,}"
)
...
# ==============================================================================
#
# Prefer the already-created item_feature_df from evidence construction.
# Otherwise load the exact saved Cell-19 evidence CSV.
# ==============================================================================

if "item_feature_df" in globals():
    ...
    if not os.path.exists(
        ...
        raise FileNotFoundError(
            "Saved item-evidence table was not found:\n"
            f"{ITEM_EVIDENCE_PATH}"
        )

    item_evidence_40 = pd.read_csv(
        ITEM_EVIDENCE_PATH
    )
...
if missing_item_cols:
    ...
    raise RuntimeError(
        "Item-evidence table is missing columns: "
        +
        ", ".join(
            sorted(
                missing_item_cols
...
if gate_stats_40[
    ...
    raise RuntimeError(
        "Validation hotels missing from item map: "
        +
        str(
            bad_items_40[:10]
        )
...
print(
    f"Joined validation cases  : "
    f"{len(case_base_40):,}"
)


# ==============================================================================
# 8. DETERMINISTIC CASE SELECTION
# ==============================================================================
#
# Fixed percentile rule:
#
#   Low gate    -> nearest to 10th percentile
#   Median gate -> nearest to 50th percentile
#   High gate   -> nearest to 90th percentile
#
# No manual selection is performed.
# ==============================================================================

selection_quantiles_40 = [
    (
        "Low gate",
        0.10
    ),
...
for case_name_40, quantile_40 in selection_quantiles_40:

    target_gate_40 = float(
        np.quantile(
            gate_values_40,
            quantile_40
        )
    ...
    chosen_40[
        "target_gate_quantile_value"
    ] = target_gate_40


    selected_rows_40.append(
        chosen_40
    )
...
# ==============================================================================
# 10. PAPER-READY TABLE
# ==============================================================================

paper_case_40 = pd.DataFrame(
    {
        "Case":
            case_study_40[
                "case"
...
print(
    "PAPER-READY VALIDATION GATE CASE STUDY"
)
...
manifest_40 = {
    ...
    "selection_rule": {
        "method":
            "nearest_to_fixed_three_seed_mean_gate_quantiles",
    ...
    "gate_value":
        "mean evidence-guided visual gate across the three final SAVRec seeds",

    "popularity":
        "raw number of training interactions for the target hotel",

    "image_count":
        "number of valid canonical hotel images",

    "coherence":
        "normalized visual-coherence evidence used by SAVRec",
    ...
    "test_set_used_for_case_selection":
        False,
